...
class SessionService:
    ...
    async def session_init(req: Request, body: dict, response: Response = None) -> dict:
        ...
        try:
            ...
            dpop_jkt, dpop_jwk, dpop_payload = validate_key_jws(payload.get("dpop_jws"), "dpop-bind+jws", ["htm", "htu", "iat", "jti", "nonce"], payload.get("device_id"), "DPoP")
        ...
        if not session_id or not session_db or session_db.get("_session_status") != SessionStatus.ACTIVE.name:
            ...
            SESSION["state"] = SessionState.pending_dpop_bind.name
            ...
            try:
                ...
                if bind_token:
                    try:
                        bind_payload = SessionService.verify_binding_token(bind_token)
                    except Exception as e:
                        log.warning(f"Bind token validation failed: {e}")
                        # First request may not have a valid bind token yet, so we allow it to continue
            except HTTPException as e:
                await SessionDB.terminate_session(session_id)
                raise HTTPException(status_code=400, detail=f"BIK or DPoP or Bind Token JWS validation failed: {e}")
